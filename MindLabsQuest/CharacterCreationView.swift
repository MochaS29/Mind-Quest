...
struct CharacterCreationView: View {
    ...
    @State private var selectedAvatarImage = ""
            ...
            if let selectedClass = selectedClass {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 15) {
                            ForEach(selectedClass.avatarImages, id: \.self) { imageName in
                                Button(action: {
                                    ...
                                    Image(imageName)
                                        ...
                                        .background(selectedAvatarImage == imageName ? Color.mindLabsPurple.opacity(0.2) : Color.mindLabsBorder.opacity(0.3))
                                        .cornerRadius(10)
                                        .overlay(
                                            RoundedRectangle(cornerRadius: 10)
                                                .stroke(selectedAvatarImage == imageName ? Color.mindLabsPurple : Color.clear, lineWidth: 2)
                                        )
                                        .scaleEffect(selectedAvatarImage == imageName ? 1.1 : 1.0)
                                        .animation(.spring(response: 0.3), value: selectedAvatarImage)
                                }
                            }
                        }
                        .padding(.horizontal)
    ...
    var classSelectionStep: some View {
        VStack(spacing: 20) {
            if !selectedAvatarImage.isEmpty {
            ...
            Text(characterName.isEmpty ? "Your Hero" : characterName)
                .font(MindLabsTypography.title2())
                .bold()
                .foregroundColor(.mindLabsText)
    ...
    var canProceed: Bool {
        ...
        case 1:
            return !characterName.isEmpty && !selectedAvatarImage.isEmpty
        case 2:
    ...
    func createCharacter() {
        ...
        gameManager.createCharacter(
            ...
            avatar: selectedAvatarImage.isEmpty ? (selectedClass.icon) : selectedAvatarImage,
